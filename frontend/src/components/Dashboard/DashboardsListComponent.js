 ...
 * @param {props} props
 */
function DashboardsListComponent(props) {
  ...
  const onClickConfirmDelete = (dashboard) => {
    setSelectedDashboard(dashboard);
    setConfirmOpen(true);
  };

  const deleteDashboard = () => {
    props.onClickDeleteDashboard(selectedDashboard._id);
  };
  ...
  return (
    ...
    >
      <ConfirmDialog
        title={"Delete " + selectedDashboard?.title}
        open={confirmOpen}
        setOpen={setConfirmOpen}
        onConfirm={deleteDashboard}
      >
        Are you sure you want to delete the Dashboard {selectedDashboard?.title}
        ?
      </ConfirmDialog>
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {stableSort(props.dashboards, getComparator(order, orderBy))
              ...
              .map((dashboard, index) => {
                return (
                  <DashboardListRow
                    key={index}
                    dashboard={dashboard}
                    onClickDisplayDashboard={props.onClickDisplayDashboard}
...
DashboardsListComponent.propTypes = {
  dashboards: PropTypes.array,
  onClickDisplayDashboard: PropTypes.func.isRequired,
  onClickDeleteDashboard: PropTypes.func.isRequired,
